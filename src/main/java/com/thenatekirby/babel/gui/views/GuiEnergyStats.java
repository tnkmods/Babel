...
public class GuiEnergyStats extends GuiTextureView {
    ...
    public void addTooltips(List<ITextComponent> tooltips) {
        ...
        tooltips.add(new StringTextComponent("Accepting " + accepts + " FE/t").mergeStyle(TextFormatting.GRAY));
        tooltips.add(new StringTextComponent("Consuming " + consumes + " FE/t").mergeStyle(TextFormatting.GRAY));

        if (stats.getEfficiency() != 1.0f || stats.getSpeed() != 1.0f) {
            String efficiency = StringFormatting.formatNumber(stats.getEfficiency());
            tooltips.add(new StringTextComponent("Efficiency " + efficiency + "x").mergeStyle(TextFormatting.DARK_GRAY));

            String speed = StringFormatting.formatNumber(stats.getSpeed());
            tooltips.add(new StringTextComponent("Speed: " + speed + "x").mergeStyle(TextFormatting.DARK_GRAY));
        }
    }
}
